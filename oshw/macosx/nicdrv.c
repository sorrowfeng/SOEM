...
/** \file
 * \brief
 * EtherCAT RAW socket driver for macOS.
 *
 * Uses libpcap (BPF) instead of the Linux AF_PACKET raw socket. Low level
 * interface functions to send and receive EtherCAT packets. EtherCAT has the
 * property that packets are only send by the master, and the send packets
 * always return in the receive buffer. There can be multiple packets "on the
 * wire" before they return. To combine the received packets with the original
 * send packets a buffer system is installed. The identifier is put in the
 * index item of the EtherCAT header. The index is stored and compared when a
 * frame is received. If there is a match the packet can be combined with the
 * transmit packet and returned to the higher level function.
 *
 ...
 * compensate. If needed the packets from interface A are resent through
 * interface B. This layer if fully transparent for the higher layers.
 */
